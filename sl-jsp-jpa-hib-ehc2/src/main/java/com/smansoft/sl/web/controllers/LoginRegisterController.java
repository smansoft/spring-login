...
/*		
		...
		javax.cache.CacheManager cacheManager1 = jcacheProvider.getCacheManager();
		
		Cache<String, String> cache = Caching.getCache("countries", String.class, String.class);
		Cache<String, String> users = Caching.getCache("users", String.class, String.class);
*/		
	...
	public ModelAndView postUserLoginView(HttpServletRequest request, HttpServletResponse response,
			@ModelAttribute UserVO userVO, BindingResult errors, ModelMap modelMap) throws ServicesException {
		String sessionId = request.getSession().getId();
		printToolStr.debug(sessionId, PrintSfx.SFX_IN);
		printToolStr.debug(sessionId, userVO);
